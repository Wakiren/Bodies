...
bool DialogueTree::UpdateTree(float dt, Module* mod, Vector2D pos)
{
	...
	{
		Engine::GetInstance().render.get()->DrawText(activeNode->texts[i].GetString(), pos.getX() + 100, pos.getY() + 20 + 50 * i, fontSize,fontSize);
	}
...
bool DialogueTree::EventReturn(Module* mod, Vector2D pos)
{
	for (int i = 0; i < activeNode->choicesList.size(); i++)
	{
		...
		case DIALOGUE_INPUT:
			...
			Engine::GetInstance().render.get()->DrawText("Player's name:", Engine::GetInstance().window.get()->width / 3, 100, fontSize,fontSize);
			if (!Engine::GetInstance().input.get()->playerName.empty())
			{
				const char* ch_name = Engine::GetInstance().input.get()->playerName.c_str();	// SString to const char*	
				Engine::GetInstance().render.get()->DrawText(ch_name, Engine::GetInstance().window.get()->width / 3 + fontSize * 7, 100, fontSize,fontSize);
			}
